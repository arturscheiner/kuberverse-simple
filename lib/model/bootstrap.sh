function bootstrap_ssh_run() {
    ...
    scp -i "${KV_KEY_PATH}" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$script" "${host}:${remote_path}"
    
    ui_info "Executing remote bootstrap on ${host}..."
    if [ -n "$capture_file" ]; then
        ssh -i "${KV_KEY_PATH}" -t -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$host" "sudo bash ${remote_path} && rm ${remote_path}" | tee "$capture_file"
    else
        ssh -i "${KV_KEY_PATH}" -t -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$host" "sudo bash ${remote_path} && rm ${remote_path}"
    fi
}
...
function bootstrap_remote_sync() {
    ...
    ssh -i "${KV_KEY_PATH}" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$host" "mkdir -p ~/.kvkit"
    scp -i "${KV_KEY_PATH}" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "${CONFIG_FILE}" "${host}:~/.kvkit/config"
    
    # Optional: Clone repo to remote host
    ui_info "Setting up kvkit tools on remote host..."
    ssh -i "${KV_KEY_PATH}" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$host" "mkdir -p ~/.kvkit/bin && if [ ! -d ~/.kvkit/bin/kuberverse-simple ]; then git clone https://github.com/arturscheiner/kuberverse-simple.git ~/.kvkit/bin/kuberverse-simple; else cd ~/.kvkit/bin/kuberverse-simple && git pull; fi"
}
...
function bootstrap_local_setup() {
    ...
    ssh -i "${KV_KEY_PATH}" -t -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$host" "sudo cp /etc/kubernetes/admin.conf /tmp/kvkit_admin.conf && sudo chmod 644 /tmp/kvkit_admin.conf"
    
    # Download the temporary file
    scp -i "${KV_KEY_PATH}" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "${host}:/tmp/kvkit_admin.conf" "${HOME}/.kube/config"
    chmod 600 "${HOME}/.kube/config"
    
    # Cleanup remote temporary file (uses -t for sudo password if needed)
    ssh -i "${KV_KEY_PATH}" -t -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "$host" "sudo rm /tmp/kvkit_admin.conf"
    
    # 2. Grab kubectl from control-plane
    ui_info "Grabbing kubectl from ${host}..."
    mkdir -p "${bin_dir}"
    scp -i "${KV_KEY_PATH}" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes "${host}:/usr/bin/kubectl" "${bin_dir}/kubectl"
